...
const DUAL_MEAS = 'عرض اليد'; // قياس ذو خانتين مضروبتين ببعض
/* بناء صفوف القياسات */
function buildMeasRows() {
  $('measRows').innerHTML = MEAS_FIELDS.map((m, i) => {
    const isDual = m === DUAL_MEAS;
    const middle = isDual
      ? `<div class="meas-two">
           <input type="number" step="0.5" inputmode="decimal" id="meas-v-${i}" placeholder="0">
           <span class="x">×</span>
           <input type="number" step="0.5" inputmode="decimal" id="meas-v2-${i}" placeholder="0">
         </div>`
      : `<input type="number" step="0.5" inputmode="decimal" id="meas-v-${i}" placeholder="0">`;
    return `
    <div class="meas-row">
...
function getMeasurements() {
  ...
  MEAS_FIELDS.forEach((m, i) => {
    ...
    const rec = { v: v === '' ? '' : Number(v), f: f === '—' ? '' : f };
    if (m === DUAL_MEAS) {
      const v2 = $('meas-v2-' + i).value;
      rec.v2 = v2 === '' ? '' : Number(v2);
    }
    out[m] = rec;
  });
...
function setMeasurements(meas) {
  MEAS_FIELDS.forEach((m, i) => {
    ...
    if (m === DUAL_MEAS) {
      const el = $('meas-v2-' + i);
      if (el) el.value = d.v2 === '' || d.v2 == null ? '' : d.v2;
    }
    const target = d.f || '—';
    $('meas-f-' + i).querySelectorAll('button').forEach(b => b.classList.toggle('on', b.dataset.f === target));
  });
}
function measText(d, key) {
  if (!d) return '—';
  const hasV = !(d.v === '' || d.v == null);
  const hasV2 = !(d.v2 === '' || d.v2 == null);
  if (key === DUAL_MEAS) {
    if (!hasV && !hasV2 && !d.f) return '—';
    const main = (hasV ? d.v : '') + (hasV2 ? ' × ' + d.v2 : (hasV ? '' : ''));
    return main + (d.f ? ' ' + d.f : '');
  }
  if (!hasV && !d.f) return '—';
  return (hasV ? d.v : '') + (d.f ? ' ' + d.f : '');
}
function measHTML(d, key) {
  if (!d) return '—';
  const hasV = !(d.v === '' || d.v == null);
  const hasV2 = !(d.v2 === '' || d.v2 == null);
  if (key === DUAL_MEAS) {
    if (!hasV && !hasV2 && !d.f) return '—';
    return (hasV ? d.v : '') + (hasV2 ? ' <span class="times">×</span> ' + d.v2 : '') + (d.f ? ' ' + esc(d.f) : '');
  }
  if (!hasV && !d.f) return '—';
  return (hasV ? d.v : '') + (d.f ? ' ' + esc(d.f) : '');
}
...
function resetInvoiceForm() {
  ...
  clearChips('neckQallabStyle'); clearChips('neckQallabButton'); clearChips('neckSadaButton');
  $('neckQallabNum').value = ''; $('neckSadaNum').value = '';
...
function loadInvoiceIntoForm(invId, asCopy) {
  ...
  if (d.neckMain) {
    ...
    if (d.neckMain === 'قلاب') {
      ...
      $('neckQallabNum').value = d.neckNumQ || '';
      setChips('neckQallabButton', d.neckButtonQ || []);
    } else {
      $('neckSada').style.display = '';
      $('neckSadaNum').value = d.neckNumS || '';
      setChips('neckSadaButton', d.neckButtonS || []);
    }
  }
...
function saveInvoice() {
  ...
  const details = {
    ...
    neckNumQ: $('neckQallabNum').value.trim(),
    neckButtonQ: chipValue('neckQallabButton'),
    neckNumS: $('neckSadaNum').value.trim(),
    neckButtonS: chipValue('neckSadaButton'),
    pocket: chipValues('pocketOpts'),
    sleeve: chipValue('sleeveOpts'),
    kabk: chipValues('kabkOpts'),
    jabzurText: $('jabzurText').value.trim()
  };
...
function neckSummary(d) {
  if (!d || !d.neckMain) return '';
  if (d.neckMain === 'قلاب') {
    const parts = ['قلاب'];
    if (d.neckStyle) parts.push(d.neckStyle);
    if (d.neckNumQ) parts.push('رقم ' + d.neckNumQ);
    if (d.neckButtonQ) parts.push(d.neckButtonQ);
    return parts.join(' — ');
  }
  const parts = ['سادة'];
  if (d.neckNumS) parts.push('رقم ' + d.neckNumS);
  if (d.neckButtonS) parts.push(d.neckButtonS);
  return parts.join(' — ');
}
function sleeveSummary(d) {
  if (!d || !d.sleeve) return '';
  return d.sleeve + (d.sleeve === 'كبك' && (d.kabk || []).length ? ' (' + d.kabk.join(' + ') + ')' : '');
}

function detailsLines(inv) {
  const d = inv.details || {};
  const lines = [];
  const neck = neckSummary(d);
  if (neck) lines.push(['نوع الرقبة', neck]);
  if ((d.pocket || []).length) lines.push(['نوع الجيب', d.pocket.join(' + ')]);
  const sleeve = sleeveSummary(d);
  if (sleeve) lines.push(['نوع الأكمام', sleeve]);
  if (d.jabzurText) lines.push(['الجبزور', d.jabzurText]);
...
function buildInvoiceHTML(inv, opts) {
  ...
  const neckAnn = neckSummary(inv.details);
  const sleeveAnn = sleeveSummary(inv.details);
  const measRows = MEAS_FIELDS.map(m => `<th>${m}</th>`).join('');
  const measVals = MEAS_FIELDS.map(m => {
    let ann = '';
    if (m === 'طول اليد' && sleeveAnn) ann = `<span class="anno">${esc(sleeveAnn)}</span>`;
    if (m === 'الرقبة' && neckAnn) ann = `<span class="anno">${esc(neckAnn)}</span>`;
    return `<td><b>${measHTML((inv.measurements || {})[m], m)}</b>${ann}</td>`;
  }).join('');
...
async function quickPDF() { setChips('poSize', 'A5'); setChips('poOrient', 'portrait'); setChips('poContent', 'full'); await doSavePDF(); }
...
async function shareInvoice(via) {
  setChips('poSize', 'A5'); setChips('poOrient', 'portrait'); setChips('poContent', 'full');
  await doShare(via);
}
...
function initApp() {
  ...
  initChips('neckQallabStyle'); initChips('neckQallabButton'); initChips('neckSadaButton');
